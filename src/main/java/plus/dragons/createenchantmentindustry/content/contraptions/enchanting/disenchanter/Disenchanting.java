...
public class Disenchanting {
    ...
    public static Type test(ItemStack itemStack, Level level) {
        ...
        if (!EnchantmentHelper.getEnchantments(itemStack).keySet().stream().filter(enchantment -> !enchantment.isCurse()).collect(Collectors.toList()).isEmpty())
            return Type.DISENCHANT;
    ...
    public static Pair<FluidStack, ItemStack> disenchant(Type type, ItemStack stack, Level level) {
        if (type == Type.DISENCHANT) {
            ...
            throw new IllegalArgumentException("BUilT-IN DISENCHANTING Type requires special handling and it should not be passed in this method.");
